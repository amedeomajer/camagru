<?php
	...
	if (isset($_GET['load_index'])){
		...
		$sql = "SELECT `id`, `user_id` FROM `user_images` ORDER BY `upload_date` DESC LIMIT $fetch_image_index , 4";
		...
		if (!empty($images))
		{
			foreach($images as $image) {
				...
				$sql = "SELECT `userr_name` FROM `user_info` WHERE `id` =".$image['user_id'].";";
				$statement = $pdo->query($sql);
				$user = $statement->fetch(PDO::FETCH_ASSOC);
				echo'<table class="gallery">
				<tr id="tr-username">
				<th>' . $user["userr_name"] . '</th>
				</tr>
				<tr>
				<td>
				<img src="' . $image['id'] . '">
				...
				echo '		<input type="submit" name="like" value="like">
						...
						<textarea name="redirect" hidden>hPage</textarea>
				</form>
				</td>
				</tr>
				<tr>
				<td>
				<form id="comments-form" action="comments.php?p=h" method="POST">
